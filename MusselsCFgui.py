...
seacarbIN : 5xn matrix with:
            ...
            seacarbIN[1,:] : DIC carbon concentration micromol/l
            seacarbIN[2,:] : TA alkalinity concentration 1e-6 eq/l
...
def SaveDataCarbonExcel(CarbonOUT,URL,name):
    #save data
    URLname=URL+name
    SheetName = ['L (mm)','Alkalinity (10^-2 mol)','TA conc. (10^-6 mol per kg)',
                 'DIC (g)','CO2 (g)','Relative CO2 Budget (g per kg)','DIC conc. (10^-6 mol per kg)','Psi','ModelInputs']
    ...
    with pd.ExcelWriter(URLname) as writer:
        for sheetN in range(len(SheetName)):
            ...
            elif (sheetN > 2 and sheetN < 6) or sheetN == 0:
    ...
    with pd.ExcelWriter(URL+'TotalConcentrations'+name) as writer:
        namesCon = ['L','DIC conc.','TA conc.','SST','Sal','Dates']
...
def SaveSeacarbExcel(seacarbOUT,seacarbIN,URLname):
    ...
    NamesInputs = ['L','DIC conc.','TA conc.','SST','Sal','Dates']
...
return: Array 6xn where rows are: ['L','DIC conc.','TA conc.','SST','Sal','Dates']
"""
...
def plotCarbonOut(CarbonOUT,XlabelOp,options):
    ...
    TextTitle = 'Seeding date (M-D): ' + '{:%m-%d}'.format(TT[0])
    OptionVar = ['Alkalinity ($10^{-2}$ mol)','$[\Delta TA]$ ($\mu$mol/kg)',
                 '$\Delta DIC$ (g)','CO$_2$ (g)','CO$_2$ (g/kg)','$[\Delta DIC]$ ($\mu$mol/kg)','$\Psi$']
    NamesAlk = ['MusselFlesh','InorganicShell','OrganicShell','Ammonium Excretion','BudgetFaeces','Total']           
    NamesAlkcon = ['MusselFlesh','InorganicShell','OrganicShell','Ammonium Excretion','BudgetFaeces','Total']                   
    NamesC = ['OrganicFlesh','OrganicShell','InorganicShell','Respiration','DRDOC',
              'Shell Respiration','Shell DRDOC','OrganicFaeces','BudgetFaeces','Total']
    ...
    if XlabelOp == 'L':
        ...
        if options == '[TA]' or options == '[DIC]' or options == 'Psi':
            Istart = 0
        Nxlabel = '$L$ (mm)'
...
def plotSeacarbOut(seacarbOUT,seacarbIN,XlabelOp,options):
    ...
    T_interval = int(np.round(len(L)/10)) # to have 10 ticklabel
    TextTitle = 'Seeding date (M-D): ' + '{:%m-%d}'.format(TT[0])
    #OptionVar = ['pH','pCO2 (ppm)']
    NamespH = ['pH','pH$_0$', '$\Delta$pH']
    NamespCO2 = ['pCO2 (ppm)','pCO2$_0$ (ppm)', '$\Delta$pCO2 (ppm)']
    OptionVar = [NamespH,NamespCO2]
...
def PlotSeacarbIn(seacarbIN,XlabelOp,options):
    ...
    T_interval = int(np.round(len(L)/10)) # to have 10 ticklabel
    TextTitle = 'Seeding date (M-D): ' + '{:%m-%d}'.format(TT[0])
    LabelName = ['$[DIC]$ $\mu$mol/kg','$[TA]$ $\mu$mol/kg','$SST$ ($^{\circ}$C)','Salinity']
    if XlabelOp == 'L':
        X = L
        Nxlabel = '$L$ (mm)'
        if L[0]<50 and L[-1]>50:
            ...
            if options == 'all':
                for i in range(4):
                    ...
                    if np.min(seacarbIN[1+i,:])*np.max(seacarbIN[1+i,:]) < 0:
                        plt.plot(X,np.zeros(np.size(X)),'r--',linewidth=1)
                    plt.plot([X[indL50],X[indL50]],[np.min(seacarbIN[1+i,:]),seacarbIN[1+i,indL50]],'b:',linewidth=1)
                    plt.plot(X[:indL50],np.ones(np.size(X[:indL50]))*seacarbIN[1+i,indL50],'b:',linewidth=1)
                    plt.xlabel(Nxlabel)
                    plt.ylabel(LabelName[i])
                    plt.title(TextTitle)
                plt.show()
            else:
                OptionVar = ['[DIC]','[TA]','SST','sal']
                VP = OptionVar.index(options) 
                plt.figure()
                plt.plot(X,seacarbIN[1+VP,:],'k-',linewidth=2)
                if np.min(seacarbIN[1+VP,:])*np.max(seacarbIN[1+VP,:]) < 0:
                    plt.plot(X,np.zeros(np.size(X)),'r--',linewidth=1)
                plt.plot([X[indL50],X[indL50]],[np.min(seacarbIN[1+VP,:]),seacarbIN[1+VP,indL50]],'b:',linewidth=1)
                plt.plot(X[:indL50],np.ones(np.size(X[:indL50]))*seacarbIN[1+VP,indL50],'b:',linewidth=1)
                plt.xlabel(Nxlabel)
                plt.ylabel(LabelName[VP])
                plt.title(TextTitle)
                plt.show()
...
frameC = tk.LabelFrame(root,text="Carbon System",font=("Helvetica", 14),bg='White', padx=10,pady=10)
...
def PlotSINop(Msimulation,Nsimulation):
    ...
    PlotSIN_button = tk.Button(frameSINout, text="Plot Selection",
    ...
    PlotSINin_button = tk.Button(frameSINin, text="Plot Selection",
                               height=2, width=30,bg=BColor,font=("Helvetica", 12),
...
frameCO = tk.LabelFrame(frameC,text="Simulation Options",font=("Helvetica", 14),bg=CF, padx=10,pady=10)
...
def SimulateCarbon(Msimulation,SCoption,Rea,Pburial,Pdrdoc,Pirdoc,pCO2_0,ConOpt):
    global CarbonOUT
    if Pburial<0 or Pburial>1:
        messagebox.showwarning("Pburial rate", "The Pburial rate should be between 0 and 1") 
    if Pdrdoc<0 or Pdrdoc>1:
        messagebox.showwarning("Pdrdoc rate", "The Pdrdoc rate should be between 0 and 1") 
    if Pirdoc<0 or Pirdoc>1:
        messagebox.showwarning("Pirdoc rate", "The Pirdoc rate should be between 0 and 1") 
...
def PlotCarbon(CarbonOUT):
    ...
    # Select variable option to plot 
    OptionVar = ['[TA]','cumDIC','cumCO2','rCO2','[DIC]','Psi']
    ...
    # button to plot the selected realizations inputs
    PlotCin_button = tk.Button(frameCPin, text="Plot Selection",
                               height=2, width=30,bg=BColor,font=("Helvetica", 12),
...
def computeCarbonVar(Msimulation):
    ...
    Ccon0_label = tk.Label(frameCin,text="Initial pCO\u2082 (ppm)",bg=CF)
    ...
    frameCsim = tk.LabelFrame(simCop,text="Simulate Carbon System:",font=("Helvetica", 14),bg=CF,padx=10,pady=10)
...
def PlotSeaCarb(seacarbOUT,seacarbIN):
    ...
    PlotC_button.grid(row=11,column=0,padx=(20,20),pady=(10,0),columnspan=2)
    # Select input to plot 
    OptionVarCIn = ['[DIC]','[TA]','SST','sal','all']
    ...
    varCInPlot_label.grid(row=9,column=0,padx=(10,0),pady=(10,0))
    # button to plot the selected realizations inputs
    PlotCin_button = tk.Button(frameSCPin, text="Plot Selection",
                               height=2, width=30,bg=BColor,font=("Helvetica", 12),
...
def computeSeaC(seacarbIN):
    ...
    Ccon0_label = tk.Label(simSeaC,text="Initial pCO\u2082 (ppm)",bg=CF)
