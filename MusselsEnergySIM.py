...
ALKcon0 : initial Alkalinity concentration 1e-6 eq/l
SST     : sea surface temperature (Celsious)
Sal     : salinity
"""
def seacarbBioCal(pCO20,ALKcon0,SST,Sal):
    ...
    INcarb_df = pd.DataFrame(columns=["pCO2_0","Alk_0"])
    INcarb_df.loc[0]=[pCO20,ALKcon0]
...
AlkCon_0  : Initial Alkalinity Concentration (10^-6 eq/l)
ConOpt    : Options to compute concentrations: ['Daily','Cumulative','Final']
"""
def MusselCarbonContent(Msimulation,SCoption,Rea,Pburial,Pdrdoc,Pirdoc,pCO2_0,AlkCon_0,ConOpt):
    ...
    Psi = seacarbBioCal(pCO2_0,AlkCon_0,SST,sal)
    ...
    AlkalinityExcretion_v = cumExc/14*100
    VarLENindex.append(NULLvarLEN)
...
seacarbIN : 5xn matrix with:
            ...
            seacarbIN[5,:] : Dates
"""
def seacarbPY(Ccon0,ALKcon0,seacarbIN):
    OUTcarb = np.zeros([4,len(seacarbIN[0,:])])
    # seacarb inputs
    DIC=(seacarbIN[1,:])*1e-6
    ALK=(ALKcon0+seacarbIN[2,:])*1e-6
    S=seacarbIN[4,:] 
    T=seacarbIN[3,:]
    # save inputs (vectors)
...
def SaveDataCarbonExcel(CarbonOUT,URL,name):
    #save data
    URLname=URL+name
    SheetName = ['L (mm)','Alkalinity (10^-2 eq)','Alkalinity (10^-6 eq per l)',
                 'Carbon (g)','CO2 (g)','CarbonFootprint (g per kg)','Carbon 10^-6 mol per l','Psi','ModelInputs']
...
def plotCarbonOut(CarbonOUT,XlabelOp,options):
    ...
    TextTitle = 'Seeding date (M-D): ' + '{:%m-%d}'.format(TT[0])
    OptionVar = ['Alkalinity ($10^{-2}$ eq)','Alkalinity ($10^{-6}$ eq/l)',
                 'Carbon (g)','CO$_2$ (g)','CarbonFootprint (g/kg)','Carbon ($10^{-6}$ mol/l)','$\Psi$']
    NamesAlk = ['MusselFlesh','InorganicShell','OrganicShell','Ammonium Excretion','BudgetFaeces','Total']           
    NamesAlkcon = ['MusselFlesh','InorganicShell','OrganicShell','Ammonium Excretion','BudgetFaeces','Total']                   
    NamesC = ['OrganicFlesh','InorganicShell','OrganicShell','Respiration','DRDOC',
              'Shell Respiration','Shell DRDOC','OrganicFaeces','BudgetFaeces','Total']
...
def PlotSeacarbIn(seacarbIN,XlabelOp,options):
    ...
    T_interval = int(np.round(len(L)/10)) # to have 10 ticklabel
    TextTitle = 'Seeding date (M-D): ' + '{:%m-%d}'.format(TT[0])
    LabelName = ['[C] $\mu$mol/l','[Alk] $10^{-6}$ eq/l','$SST$ ($^{\circ}$C)','Salinity']
    if XlabelOp == 'L':
        X = L
        Nxlabel = '$L$ (mm)'
        # plot the variables 
...
def SimulateCarbon(Msimulation,SCoption,Rea,Pburial,Pdrdoc,Pirdoc,pCO2_0,AlkCon_0,ConOpt):
    global CarbonOUT
    CarbonOUT = MusselCarbonContent(Msimulation,SCoption,Rea,Pburial,Pdrdoc,Pirdoc,pCO2_0,AlkCon_0,ConOpt)
    ### close the window
    ### simCop.destroy()
...
def PlotCarbon(CarbonOUT):
    ...
    # Select variable option to plot 
    OptionVar = ['Alk','[Alk]','cumC','cumCO2','CF','[C]','Psi']
...
def computeCarbonVar(Msimulation):
    ...
    RUNcarbon_button = tk.Button(frameCsim, text="Obtain Carbon Variables",
                           ...
                           comman=lambda: SimulateCarbon(Msimulation,SCclicked.get(),NsimSC.get(),
                                                         ...
                                                         float(Ccon0.get()),float(Alkcon0.get()),ConClicked.get()))
...
def seacarbSIM(Ccon0,ALKcon0,seacarbIN):
    SimSeaState = tk.Label(frame2SC,text="Running ... ",width=20)
    SimSeaState.grid(row=1,column=1)
    global seacarbOUT
    start_time_sim = time.process_time()
    seacarbOUT = seacarbPY(Ccon0,ALKcon0,seacarbIN)
    end_time_sim = time.process_time()
    Tspent = end_time_sim - start_time_sim 
    ## message end simulation
    if Tspent < 10:
...
def computeSeaC(seacarbIN):
    ...
    SimPH_button = tk.Button(frame2SC, text="Start Simulation",width=20,bg=BColor,
                         comman=lambda:seacarbSIM(float(Ccon0.get()),float(Alkcon0.get()),seacarbIN))
